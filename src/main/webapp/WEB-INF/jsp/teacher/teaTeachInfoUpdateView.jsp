...
<div class="tab_space_add">
   <div class="tab_space_add_title"><h1>教学情况更改</h1></div>
       ...
       <table class="table_add_content_title">
        ...
        <form name="work_form" method="POST" action="/teacher/submitTeachInfo">
            ...
            <div class="row fixWidth">
                <input name="tc[1].tcId" type="hidden" value="${teaCoInfo.tcId}">
                <%--学期--%>
                <div class="row_1">
                ...
                <div class="row_3">
                    <select name="tc[1].coName" onchange="coNameChanged(this);">
                        <c:forEach var="course" items="${teacher.courses}">
                            <option value="${course.coNO}" ${teaCoInfo.coName==course.coNO?"selected":""}>${course.coName}</option>
                        </c:forEach>
                    </select>
                </div>
                <%--总人数--%>
                ...
                <%--班级批次--%>
                <div class="row_6">
                    <input type="number" name="tc[1].houseNum" required min="0" value="${teaCoInfo.houseNum}"/>
                </div>
                <%--未通过人数--%>
                <div class="row_7">
                ...
                <%--备注--%>
                <div class="row_10">
                    <input type="text" name="tc[1].mark" required value="${teaCoInfo.mark}"/>
                </div>
                <%--操作--%>
                <div class="row_11">
                    <div class="same add">
            ...
            <%int i=1;%>
            <c:forEach var="tc" items="${teaCoInfos}">
                <%i++;%>
                <div class="row fixWidth">
                    <input name="tc[<%=i%>].tcId" type="hidden" value="${tc.tcId}">
                        <%--学期--%>
                    <div class="row_1">
                        <input class="radio_btn" name="tc[<%=i%>].term" type="radio" value="春"${tc.term=="春"?"checked":""}/><label>春</label>
                        <input class="radio_btn" name="tc[<%=i%>].term" type="radio" value="秋"${tc.term=="秋"?"checked":""}/><label>秋</label>
                    </div>
                        <%--课程类型--%>
                    <div class="row_2">
                        <select name="tc[<%=i%>].type">
                            <option value="理论教学" ${tc.type=="理论教学"?"selected":""}>理论教学</option>
                            <option value="课程设计" ${tc.type=="课程设计"?"selected":""}>课程设计</option>
                            <option value="助教" ${tc.type=="助教"?"selected":""}>助教</option>
                            <option value="自行上课" ${tc.type=="自行上课"?"selected":""}>自行上课</option>
                        </select>
                    </div>
                        <%--课程名称--%>
                    <div class="row_3">
                        <select name="tc[<%=i%>].coName" onchange="coNameChanged(this);">
                            <c:forEach var="course" items="${teacher.courses}">
                                <option value="${course.coNO}" ${tc.coName==course.coNO?"selected":""}>${course.coName}</option>
                            </c:forEach>
                        </select>
                    </div>
                        <%--总人数--%>
                    <div class="row_4">
                        <input type="tel" name="tc[<%=i%>].stuNum" required min="0" value="${tc.stuNum}" readonly/>
                    </div>
                        <%--课程学时--%>
                    <div class="row_5">
                        <input name="tc[<%=i%>].coTime" type="tel" readonly="readonly" value="${tc.coTime}">
                    </div>
                        <%--班级批次--%>
                    <div class="row_6">
                        <input type="number" name="tc[<%=i%>].houseNum" required min="0" value="${tc.houseNum}"/>
                    </div>
                        <%--未通过人数--%>
                    <div class="row_7">
                        <input type="tel" name="tc[<%=i%>].notPassNum" required min="0" value="${tc.notPassNum}"
                               readonly/>
                    </div>
                        <%--课程学分--%>
                    <div class="row_8">
                        <input type="tel" name="tc[<%=i%>].coScore" min="0" readonly value="${tc.coScore}"/>
                    </div>
                        <%--双语教学--%>
                    <div class="row_9">
                        <input class="radio_btn" name="tc[<%=i%>].co2Language" value="true" ${tc.co2Language==true?"checked":""} type="radio" /><label>是</label>
                        <input class="radio_btn" name="tc[<%=i%>].co2Language" value="false" ${tc.co2Language==false?"checked":""} type="radio"/><label>否</label>
                    </div>
                        <%--备注--%>
                    <div class="row_10">
                        <input type="text" name="tc[<%=i%>].mark" required value="${tc.mark}"/>
                    </div>
                        <%--操作--%>
                    <div class="row_11">
                        <div class="same add">
