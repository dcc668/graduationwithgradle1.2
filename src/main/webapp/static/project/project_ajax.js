...
$(function(){
    var trs=$(".table_report tr")
    $.each(trs,function(idx,obj){
        if(idx!=0){
            var tds=$(obj).children(".col_2");
            $.each(tds, function (idx2,obj2) {
                //列文本
                var tdVal=$(obj2).html();
                //要填充的值
                var val=$(obj2).next().find("input").attr("value");
                insertValToTab2(tdVal,val);
